...
export default function Customers() {
  ...
  const getCustomers = async () => {
    let response = await axios.get(
      `http://127.0.0.1:8000/api/admins/costumers/${pagination}`,
      {
        headers: {
  ...
  const deleteCustomer = async (id) => {
    await axios.delete(
      `http://127.0.0.1:8000/api/admins/customers/${id}`,
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
          "Content-Type": "application/json",
  ...
  const search = async () => {
    let response = await axios.get(
      `http://127.0.0.1:8000/api/admins/${typeSearch}/?query=${inputSearch}`,
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
          "Content-Type": "application/json",
